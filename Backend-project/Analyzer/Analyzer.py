...
def get_recommend_list():
    ...
    for heat_game in heat_games:
        ...
        emoji=game.emoji
        emoji=emoji.split(',')
        weight+=emoji[0]/emoji[1]*10
        weight+=heat_game['stat']
        heat_game['weight']=weight
    return sorted(heat_games,key=lambda x:x[weight])
...
def getHotTable(list_name):
    ...
    if (list_name == "played_list"):
        list_games=[]
        played_games = game_list.query.filter(game_list.played_rank != 0).order_by(game_list.played_rank).all()  # 取出热榜游戏
        for played_game in played_games:
            list_game = {"game_name": "", "stat": "", "category_name": ""}
    ...
    if (list_name == "reserved_list"):
        list_games=[]
        reserved_games = game_list.query.filter(game_list.reserved_rank != 0).order_by(game_list.reserved_rank).all()  # 取出热榜游戏
        for reserved_game in reserved_games:
            list_game = {"game_name": "", "stat": "", "category_name": ""}
