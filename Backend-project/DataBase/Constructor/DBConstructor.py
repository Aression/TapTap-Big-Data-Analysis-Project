...
BaseInfoPath = 'DataBase/Constructor/Data/BaseInfo'
BaseRankPath = 'DataBase/Constructor/Data/Rank'
assert os.path.exists(BaseInfoPath)
assert os.path.exists(BaseRankPath)
...
def ConstructByAutoErgodic():
    ...
    for filepath, dirs, filenames in os.walk(BaseInfoPath):
        for filename in filenames:
            data = pd.read_csv("{}/{}".format(filepath, filename))
            # data=pd.read_csv("D:/TapTap-Big-Data-Analysis-Project/Backend-project/DataBase/Constructor/Data
            # /BaseInfo/CategoryDetailsSpider_mo_ni.csv")
            for index, RowData in data.iterrows():
